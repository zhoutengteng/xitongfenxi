...
CREATE TABLE `xitongfenxi`.`users` (
  ...
  `password` VARCHAR(45) NOT NULL DEFAULT '' COMMENT 'MD5加密',
  `mail` VARCHAR(45) NOT NULL DEFAULT '',
  `phone` VARCHAR(45) NOT NULL DEFAULT '',
  `headImageAdd` VARCHAR(45) NOT NULL DEFAULT '',
  `addr` VARCHAR(45)  NOT NULL DEFAULT '',
  `contactPeopleList` VARCHAR(45) NOT NULL DEFAULT '' COMMENT 'usersId列表，如1,11,13 用逗号隔开',
  `rate` INT NOT NULL DEFAULT 0 COMMENT '个人信用度，越大越好',
...
CREATE TABLE `xitongfenxi`.`records` (
  ...
  `peopleId` INT NOT NULL DEFAULT 0,
  `joinPeopleList` VARCHAR(45) NOT NULL DEFAULT '',
  PRIMARY KEY (`id`),
...
CREATE TABLE `xitongfenxi`.`comments` (
  ...
  `badsCount` INT NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
